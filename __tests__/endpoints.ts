...
describe("Endpoints", () => {
  ...
  it("POST /books", async () => {
    const result = await request(app).post("/books")
    .send(postPayload);
    expect(JSON.stringify(result.text)).toEqual(`\"${postPayload.title} was added to the inventory \"`);
    expect(result.statusCode).toEqual(200);
  });

  it("DELETE /books/:isbn", async () => {
    const result = await request(app).delete(`/books/${isbn}`)
    expect(JSON.stringify(result.text)).toEqual(`\"successfully deleted book with isbn code ${isbn}\"`);
    expect(result.statusCode).toEqual(200);
  });

  it("PATCH /books/:isbn", async () => {
    const result = await request(app).patch(`/books/${putPayload.isbn}`)
    .send(putPayload)
